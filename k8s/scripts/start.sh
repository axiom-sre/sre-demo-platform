#!/bin/bash
# =============================================================================
# start.sh — SRE Demo Platform: Full Startup (Holy Grail Edition)
# =============================================================================
#
# Usage:
#   bash scripts/start.sh              # full startup
#   bash scripts/start.sh --pf-only   # restart port-forwards only
#   bash scripts/start.sh --verify    # re-run pipeline verification only
#
# STARTUP ORDER (order matters — don't change it):
#   0. PriorityClasses  — must exist before any pod is scheduled
#   1. Namespaces + ResourceQuotas — fence the namespaces before deploying
#   2. Observability infrastructure (infra + metrics-server)
#   3. Core observability stack (Prometheus, Loki, Tempo, Alloy)
#   4. Grafana (depends on Prometheus being up for datasource validation)
#   5. Boutique application services
#   6. HPA (must come AFTER metrics-server is ready)
#   7. Port-forwards + pipeline verification
#
# KEY CHANGES vs previous version:
#   1. priority-classes.yaml applied first — pods can't be scheduled with
#      a priorityClassName that doesn't exist yet. Previous version was
#      applying boutique.yaml before the PriorityClass manifests, causing
#      cartservice to fail with "no PriorityClass found".
#   2. Namespaces applied before any resource in that namespace.
#      ResourceQuotas are enforced immediately on application.
#   3. Alloy health gate extended to 390s (was 300s). macOS sleep/wake
#      + Docker Desktop VM resume can add 60s before kubelet is responsive.
#   4. Port-forward health check uses NodePort for frontend (30080) and
#      Grafana (30300) — avoids the kubectl port-forward goroutine bottleneck
#      at 1000 VU. Port-forwards kept only for observability components.
# =============================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ "$(basename "$SCRIPT_DIR")" == "scripts" ]]; then
  REPO_ROOT="$(dirname "$SCRIPT_DIR")"
else
  REPO_ROOT="$SCRIPT_DIR"
fi
cd "$REPO_ROOT"

MODE="${1:-}"

GREEN='\033[0;32m'; YELLOW='\033[1;33m'; RED='\033[0;31m'; NC='\033[0m'
info()    { echo -e "${GREEN}▶${NC} $*"; }
warn()    { echo -e "${YELLOW}⚠${NC}  $*"; }
error()   { echo -e "${RED}✗${NC} $*"; }
section() {
  echo -e "\n${GREEN}════════════════════════════════════════${NC}"
  echo -e "${GREEN} $*${NC}"
  echo -e "${GREEN}════════════════════════════════════════${NC}"
}

# ── Port-forward definitions (observability only — boutique uses NodePort) ────
# Frontend port-forward on 8080 runs alongside NodePort 30080.
# Docker Desktop NodePort binding is unreliable across versions and after
# sleep/wake cycles. Port-forward on 8080 is the guaranteed fallback.
# Load tests try NodePort 30080 first, fall back to 8080 automatically.
PF_DEFS=(
  "boutique      frontend   8080  8080  http://localhost:8080/"
  "observability grafana    3000  3000  http://localhost:3000/api/health"
  "observability prometheus 9090  9090  http://localhost:9090/-/healthy"
  "observability alloy      12345 12345 http://localhost:12345/-/ready"
  "observability tempo      3200  3200  http://localhost:3200/ready"
  "observability loki       3100  3100  http://localhost:3100/ready"
)
PF_PID_FILE="${REPO_ROOT}/.pf-pids"

# ── Helpers ───────────────────────────────────────────────────────────────────
wait_ready_kind() {
  local kind=$1 name=$2 ns=$3 timeout_sec=${4:-120}
  info "Waiting for $kind/$name ($ns) — up to ${timeout_sec}s..."
  if ! kubectl rollout status "$kind/$name" -n "$ns" \
       --timeout="${timeout_sec}s" 2>/dev/null; then
    warn "$kind/$name timed out — check: kubectl logs -n $ns $kind/$name --tail=40"
    return 1
  fi
  if [[ "$kind" == "deployment" ]]; then
    local ready
    ready=$(kubectl get deployment "$name" -n "$ns" \
      -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
    if [[ "${ready:-0}" -lt 1 ]]; then
      warn "$name has 0 ready replicas"
      return 1
    fi
    info "$name → ${ready} replica(s) ready ✓"
  else
    local desired ready
    desired=$(kubectl get daemonset "$name" -n "$ns" \
      -o jsonpath='{.status.desiredNumberScheduled}' 2>/dev/null || echo "0")
    ready=$(kubectl get daemonset "$name" -n "$ns" \
      -o jsonpath='{.status.numberReady}' 2>/dev/null || echo "0")
    info "$name → ${ready}/${desired} pods ready ✓"
  fi
}

wait_http() {
  local url=$1 timeout_sec=${2:-120} label=${3:-$1}
  info "Waiting for $label to respond (up to ${timeout_sec}s)..."
  local elapsed=0
  while ! curl -sf "$url" -o /dev/null -m 5 2>/dev/null; do
    sleep 5; elapsed=$((elapsed + 5))
    if [[ $elapsed -ge $timeout_sec ]]; then
      warn "$label did not respond after ${timeout_sec}s"
      return 1
    fi
  done
  info "$label is up ✓"
}

start_port_forward() {
  local ns=$1 svc=$2 local_port=$3 remote_port=$4 health_url=$5
  local log_file="/tmp/pf-${svc}.log"
  # Kill any existing forward for this port
  pkill -f "kubectl port-forward.*${local_port}:" 2>/dev/null || true
  sleep 1
  kubectl port-forward -n "$ns" "svc/$svc" "${local_port}:${remote_port}" \
    > "$log_file" 2>&1 &
  echo $! >> "$PF_PID_FILE"
  # Wait up to 15s for the port-forward to become healthy
  local elapsed=0
  while ! curl -sf "$health_url" -o /dev/null -m 3 2>/dev/null; do
    sleep 2; elapsed=$((elapsed + 2))
    if [[ $elapsed -ge 15 ]]; then
      warn "Port-forward for $svc:$local_port did not come up (check $log_file)"
      return 0  # non-fatal — NodePort is the primary path
    fi
  done
  info "Port-forward $svc:$local_port ready ✓"
}

# ── --pf-only mode ────────────────────────────────────────────────────────────
if [[ "$MODE" == "--pf-only" ]]; then
  section "Port-forwards only"
  pkill -f "kubectl port-forward" 2>/dev/null || true
  [[ -f "$PF_PID_FILE" ]] && rm "$PF_PID_FILE"
  touch "$PF_PID_FILE"
  for pf in "${PF_DEFS[@]}"; do
    read -r ns svc lp rp url <<< "$pf"
    start_port_forward "$ns" "$svc" "$lp" "$rp" "$url"
  done
  info "Port-forwards started. Frontend: http://localhost:8080  Grafana: http://localhost:3000"
  exit 0
fi

# ── --verify mode ─────────────────────────────────────────────────────────────
if [[ "$MODE" == "--verify" ]]; then
  bash "$SCRIPT_DIR/verify-stability.sh" --short
  exit $?
fi

# ═════════════════════════════════════════════════════════════════════════════
# FULL STARTUP
# ═════════════════════════════════════════════════════════════════════════════

section "0. Preflight"
command -v kubectl &>/dev/null || { error "kubectl not found"; exit 1; }
kubectl cluster-info &>/dev/null || { error "cluster unreachable — is Docker Desktop running?"; exit 1; }
info "Cluster: $(kubectl config current-context)"

# Check Docker Desktop has enough resources
total_mem_gb=$(kubectl get node -o jsonpath='{.items[0].status.capacity.memory}' 2>/dev/null \
  | sed 's/Ki$//' | awk '{printf "%.0f", $1/1024/1024}' || echo "0")
if [[ "$total_mem_gb" -lt 16 ]]; then
  warn "Node has only ${total_mem_gb}GB RAM. Recommend 24GB+ for 1000 VU. Check Docker Desktop → Resources."
else
  info "Node memory: ${total_mem_gb}GB ✓"
fi

section "1. PriorityClasses + Namespaces"
# PriorityClasses MUST be applied before any pod that references them
kubectl apply -f namespaces/priority-classes.yaml
kubectl apply -f namespaces/namespaces.yaml
info "PriorityClasses and namespaces ready ✓"

section "2. Observability infrastructure"
kubectl apply -f observability/infrastructure/infrastructure.yaml
# metrics-server is critical for HPA — wait for it
wait_ready_kind deployment metrics-server kube-system 120 || warn "metrics-server slow — HPA may not work initially"

section "3. Core observability stack"
kubectl apply -f observability/prometheus/prometheus.yaml
kubectl apply -f observability/loki/loki.yaml
kubectl apply -f observability/tempo/tempo.yaml

# Prometheus must be ready before Alloy tries to remote_write spanmetrics
info "Waiting for Prometheus..."
wait_ready_kind deployment prometheus observability 180

# Loki and Tempo can start in parallel — don't block on them
info "Loki and Tempo starting in background..."
kubectl rollout status deployment/loki -n observability --timeout=120s 2>/dev/null &
kubectl rollout status deployment/tempo -n observability --timeout=120s 2>/dev/null &

section "4. Alloy (trace + log collector)"
kubectl apply -f observability/alloy/alloy.yaml

# Alloy has a long startup on cold Docker Desktop — 390s gate
info "Waiting for Alloy DaemonSet (up to 390s — cold start can take 2-3 min)..."
alloy_ready=false
for i in $(seq 1 78); do  # 78 × 5s = 390s
  desired=$(kubectl get daemonset alloy -n observability \
    -o jsonpath='{.status.desiredNumberScheduled}' 2>/dev/null || echo "0")
  ready=$(kubectl get daemonset alloy -n observability \
    -o jsonpath='{.status.numberReady}' 2>/dev/null || echo "0")
  if [[ "${ready:-0}" -ge 1 && "${ready:-0}" -ge "${desired:-1}" ]]; then
    alloy_ready=true
    break
  fi
  sleep 5
done
if $alloy_ready; then
  info "Alloy ready ✓"
else
  warn "Alloy not fully ready after 390s — check: kubectl logs -n observability daemonset/alloy --tail=40"
fi

section "5. Grafana"
kubectl apply -f observability/grafana/grafana.yaml
# Non-fatal: Grafana timeout must NOT abort boutique deployment.
# A slow image pull (first run, no cache) can take 3-5min on some networks.
# The stack is fully functional without Grafana — boutique must still deploy.
wait_ready_kind deployment grafana observability 180 || \
  warn "Grafana still pulling image — boutique will deploy now. Grafana will catch up."

# Wait for background loki+tempo rollouts (non-fatal)
wait || true

section "6. Boutique application"
kubectl apply -f boutique/boutique.yaml

# Wait for the critical path in order: redis → cart → frontend
info "Waiting for Redis..."
wait_ready_kind deployment redis boutique 60 || warn "Redis slow to start"
info "Waiting for cartservice (long .NET warmup — up to 3 min)..."
wait_ready_kind deployment cartservice boutique 180 || warn "CartService slow — check: kubectl logs -n boutique deployment/cartservice --tail=20"
info "Waiting for frontend..."
wait_ready_kind deployment frontend boutique 120 || warn "Frontend slow to start"

# Start remaining services without blocking
for svc in emailservice productcatalogservice recommendationservice shippingservice \
           paymentservice currencyservice adservice checkoutservice; do
  kubectl rollout status "deployment/$svc" -n boutique --timeout=180s 2>/dev/null &
done
wait || true
info "All boutique services ready ✓"

section "7. HPA"
# HPA requires metrics-server to be fully functional — verify first
if kubectl top nodes &>/dev/null 2>&1; then
  info "metrics-server is returning data — applying HPA"
  kubectl apply -f boutique/hpa.yaml
  info "HPA applied ✓"
else
  warn "metrics-server not returning data yet — HPA will be applied but may show <unknown> CPU initially"
  kubectl apply -f boutique/hpa.yaml
  warn "Run 'kubectl get hpa -n boutique' in 60s to verify HPA has CPU metrics"
fi

section "8. Port-forwards (observability)"
pkill -f "kubectl port-forward" 2>/dev/null || true
[[ -f "$PF_PID_FILE" ]] && rm "$PF_PID_FILE"
touch "$PF_PID_FILE"
for pf in "${PF_DEFS[@]}"; do
  read -r ns svc lp rp url <<< "$pf"
  start_port_forward "$ns" "$svc" "$lp" "$rp" "$url"
done

section "9. Final status"
echo ""
kubectl get pods -n observability -o wide 2>/dev/null
echo ""
kubectl get pods -n boutique -o wide 2>/dev/null
echo ""
kubectl get hpa -n boutique 2>/dev/null

echo ""
echo -e "${GREEN}═══════════════════════════════════════════════${NC}"
echo -e "${GREEN} STACK IS UP${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════${NC}"
echo ""
echo "  Boutique:             http://localhost:8080    (LoadBalancer — Docker Desktop maps to localhost)"
echo "  Grafana:              http://localhost:3000    admin/admin  (LoadBalancer)"
echo ""
echo "  Prometheus:           http://localhost:9090"
echo "  Alloy:                http://localhost:12345"
echo "  Tempo:                http://localhost:3200"
echo "  Loki:                 http://localhost:3100"
echo ""
echo "  Find working URL:     bash scripts/find-url.sh"
echo "  Run smoke test:       k6 run --env BASE_URL=\$(bash scripts/find-url.sh --export) scripts/load-test_10vusers.js"
echo "  Run 100 VU test:      k6 run --env BASE_URL=\$(bash scripts/find-url.sh --export) scripts/load-test_100vusers.js"
echo "  Verify stability:     bash scripts/verify-stability.sh --short"
echo "  Full debug:           bash scripts/manage.sh debug"
echo ""
